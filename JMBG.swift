...
func validateJMBG(_ jmbg: String) -> (JMBG?, String)? {
    guard jmbg.count == 13, let numericJMBG = Int(jmbg) else {
        return (nil, "Validation Failed: Incorrect length or non-numeric characters")
    }
    ...
    guard day >= 1 && day <= 31 else {
             return (nil, "Validation Failed: Invalid day in JMBG")
        }

        guard month >= 1 && month <= 12 else {
            return (nil, "Validation Failed: Invalid month in JMBG")
        }
    ...
    guard l == k else {
        return (nil, "Validation Failed: Invalid control number")
    }
    ...
    guard let dateOfBirth = dateFormatter.date(from: "\(day)/\(month)/\(year)") else {
        return (nil, "Validation Failed: Unable to parse date of birth")
    }
    ...
    return (JMBG(gender: gender, dateOfBirth: dateFormatter.string(from: dateOfBirth), region: region), "Invalid JMBG")
}

// Function to get the region name based on the provided code
...
// Function to get the city name based on the provided code
